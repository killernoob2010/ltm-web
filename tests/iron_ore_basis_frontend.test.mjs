...
test("basis pages refresh the latest stored data date on every activation", () => {
  ...
  const basisDisplay = indexHtml.match(/<section id="ironOreBasisDisplayView"[\s\S]*?(?=<section|$)/);
  assert.ok(basisDisplay);
  assert.doesNotMatch(basisDisplay[0], /异常数据|同步异常|失败次数/);
});
