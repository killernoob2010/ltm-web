...
## 11. 本地执行回执（2026-09-15）

- 已完成三个本地提交：`b8290d4`（请求范围与时间规范化）、`e6dc821`（共享引用规则与 SDK 修正反馈）、`78821b0`（源时点保护与中文原题端到端回归）。实现位于隔离工作树 `codex/agent-batch-c-six-fixes-20260915-447c`，起点为 `5115d0a`。
- `tests/agent_v2` 545 项通过；前端 Node 测试 17 项通过；运行时 `pip check` 和 `git diff --check` 通过。新增端到端测试只用临时 SQLite、合成 ToolEnvelope 和 FunctionModel，未调用真实模型/搜索。
- 正式环境保持停用，未 push、merge、部署或修改生产数据。真实管理员问答、正式源时点和生产回归仍由主 Agent 在单独发布授权后执行。
