...
现货业务台账的本地同步验收使用 `tests/fixtures/spot_ledger_sales_contract_fixture.json`，覆盖 7 个销售组、数量回退、B05/B09 原值保留及落地货关系判定、跨组标识、待补录和同步异常。销售类型 D 直接显示源系统原值，不再转换成展示标签；B05/B09（含带后缀的完整业务类别）只通过独立关系判断是否为落地货。自动同步调度仅在显式设置 `SPOT_LEDGER_AUTO_SYNC_ENABLED=true` 时启动，按北京时间 09:00—18:00 每小时执行；服务在同步时段内启动时只执行最近一个已到小时，不补跑当天此前所有小时，数据库已有同一时段记录时跳过，19:00 后启动不补跑；不提供实时同步或手动立即同步。Staging 当前使用 `SPOT_LEDGER_SOURCE_MODE=official_json` 并启用上述定时任务；本地默认不启用。
