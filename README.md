...
- 收盘交易复盘 Agent Phase 1（本地候选，未部署）：提供固定宏源账户铁矿石期权指定日期的只读摘要接口，按真实月份、Call/Put、买卖方向和行权价区间动态分组，分别返回不扣手续费的真实平仓盈亏与日结算口径持仓浮盈浮亏；日结单优先，只有月结单时明确降级为部分完成。关键数字随来源、数据时点、计算/规则版本、完整性、警告和证据引用返回；模型、企业微信、15:00 最后成交价和生产部署均未接入。
