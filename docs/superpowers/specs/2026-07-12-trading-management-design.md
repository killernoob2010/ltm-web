...
日期：2026-07-12
状态：待用户书面审阅
修订：2026-07-20 补充期权到期放弃、行权和履约的统一了结口径
...
- 事实口径：成交、普通平仓、期权到期/行权/履约、持仓及逐笔平仓盈亏的原始结果，用于总览、持仓与交易和账户核对；
...
- 普通成交平仓的事实平仓盈亏直接取平仓记录中的逐笔平仓盈亏；
- 期权到期放弃、行权和履约在业务展示上视为平仓的一种，统一进入“平仓记录”，但通过“了结类型”与普通成交平仓区分；
- 期权了结事件的期权腿核算平仓价为 `0`：买入期权盈亏为 `(0 - 开仓价) × 手数 × 合约乘数`，卖出期权盈亏为 `(开仓价 - 0) × 手数 × 合约乘数`；原始事件金额和账单盈亏另行保留，不覆盖系统核算值；
- 行权或履约必须同时关联按行权价形成的标的期货持仓。期权腿盈亏与标的期货盈亏分开保存，业务总盈亏合并计算，禁止只计算期权腿或重复计算行权内在价值；
...
新页面必须以 `/Users/wangjingze/Documents/交易台账自动化处理/personal_workbench/trading_static/` 提交 `c1de82b` 为直接实现基准。左侧保留现有系统统一导航；右侧工作区复用原型的信息结构、视觉组件、区块顺序、字段顺序和交互形式。原型不是仅供参考的视觉草图；除用户后续明确确认的业务调整外，不允许自行删减、替换或使用系统通用组件重新设计。未列入确认差异清单的页面差异均视为实现缺陷。
...
- `id`；
- `account_id`；
- `fact_type`：`trade`、`close`、`option_event`、`position`；
- `stable_key`；
- `created_at`。
...
期权行权、履约、到期放弃继续保存为独立、可追溯的原始事件，不伪造一笔市场成交；事实查询时与普通平仓合并投影到同一个“平仓记录”列表。

- 稳定身份至少包含账户、事件日期、交易所、合约、方向、事件类型、手数、行权价和同组稳定出现序号；
- `settlement_type` 取 `exercise`、`assignment` 或 `expiry_abandon`；
- 期权腿核算平仓价固定为 `0`，同时保留原始事件类型、原始金额、账单盈亏和手续费；
- 必须尽可能关联被了结的期权开仓身份；无法唯一关联时允许保存原始事件，但盈亏和业务归属标记为“待核验”；
- `exercise` 和 `assignment` 必须关联按行权价形成的标的期货持仓身份；`expiry_abandon` 不生成标的期货持仓；
- 相同日结和月结中的同一事件只形成一个当前事实版本，同时保留全部来源关系。
...
- 普通成交平仓盈亏取 `trading_close_facts`，期权了结盈亏取期权事件与其开仓关联后的系统核算值；两类共同组成统一平仓视图的事实盈亏；
- “平仓记录”是普通平仓事实与期权了结事件的统一查询视图，不要求增加独立页面；
- “了结手数”包含普通平仓、行权、履约和到期放弃；“成交平仓手数”只统计真实反向成交，二者不得混用；
...
5. 期权行权、履约和到期放弃按被关联开仓扣减业务未平手数；行权或履约形成的标的期货持仓继承该期权的业务归属并保留来源关系；
6. 无法形成完整业务关系的平仓或期权了结事件进入待归类或待核验；
7. 业务当前持仓由完整开仓成交减去普通平仓分配手数和期权了结手数得到；
8. 业务持仓必须与事实快照进行数量核验，差异显示在核验结果中，不得伪装一致。
...
期权普通成交平仓使用期权合约自身成交价格和对应乘数，方向公式与期货一致。期权行权、履约或到期放弃的期权腿使用 `0` 作为核算平仓价；行权或履约产生的标的期货持仓按行权价建仓，其后盈亏按期货口径计算。业务手续费单独展示事实匹配手续费，本期不重新分配手续费改变业务盈亏公式。
...
- 页面不新增“行权与到期”页签；现有“平仓记录”增加“了结类型”，统一显示普通平仓、行权、履约和到期放弃；
- 普通平仓显示真实平仓价，行权、履约和到期放弃显示核算平仓价 `0`；详情中可追溯原始事件，行权或履约可追溯其标的期货持仓；
- 合约搜索在回车或点击搜索后应用；
- 浮动盈亏位置统一显示“待计算”。
...
- 合约全部平仓后的事实/业务盈亏核对；
- 期权买方到期放弃按平仓价 `0` 计算全部权利金损失，期权卖方到期作废按平仓价 `0` 计算全部权利金收益；
- 行权/履约的期权腿按 `0` 了结并关联行权价标的期货持仓，验证业务总盈亏不漏算、不重复；
...
- 公共标签顺序、筛选应用和分页；
- 事实口径与业务口径标签；
- 平仓记录用“了结类型”区分普通平仓、行权、履约和到期放弃，不新增页面；
- 浮动盈亏和风险指标显示“待计算”；
- 三表缺失时确认按钮不可用；
- 批量归类只按整笔成交；
- 重配候选、影响预览和确认错误提示；
...
- 成交 2,753 条；
- 普通成交平仓 2,351 条，合并 1 条期权到期放弃后“平仓记录”共 2,352 条；
- 最新持仓 669 条；
- 逐笔平仓盈亏 3,497,480 元；
- 全部成交手续费 35,380.88 元；
- 平仓匹配手续费 16,885.34 元；
...
- 期权了结事件 1 条：`i2607-p-750` 买入 Put 100 手于 2026-06-16 到期放弃；平仓记录显示了结类型“到期放弃”、核算平仓价 `0`，不生成卖出成交，事件后对应期权持仓为 `0`；
- 该事件按原始买入权利金和合约乘数计算期权腿损失；账单事件区段中的盈亏 `0` 作为原始字段保留，不覆盖系统核算盈亏。
