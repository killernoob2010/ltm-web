...
“周报生成”页面可选择报告周和模板版本，先检查库存、实际到港、预计/估算到港、旧表需和价格的覆盖，再生成PDF。当前登记的模板为**V1.0**，对应已确认的46页版式；实际到港、来源预计、模型估算和旧表需保持独立，基差与以日照为基准的港差分开显示。每份报告绑定模板版本、数据快照和修订号，PDF使用 `铁矿石周报_<周>_模板V1.0_R<修订>.pdf` 命名，历史文件不覆盖。首版生成在本地/Staging请求内完成并记录任务状态，后续可以在不改变报告口径的前提下接入后台队列。
...
本次选择发布不包含矿石周报、V2 数据整合及订单融资新功能。现货台账支持业务来源字段与人工补录分开管理。正式接入状态以版本记录中的真实验收结果为准。
