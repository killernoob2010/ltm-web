...
2026-09-15 定向复核：M-C01—M-C04 已拆分为真实的多轮用户请求，oracle 包含合成首轮答案、
逐轮预期和撤权事件。它们是后续行为执行的输入定义，本命令仍不执行对话。
主 Agent 已复核后六条的取消、租约、跨用户引用、撤权、敏感错误和预算预期，并冻结当前
六行内容（含换行）的 SHA256：`628cc5fb0807633ebe49ea085724fc7cfd3a1b103f0c75bfe9ff7ffa189dedbe`。
这些 holdout 已向执行者公开，只能作为固定安全回归样本，不能证明未见样本泛化。
...
本候选版本另外登记了天气+发运的自然语言混合来源、明确禁止联网、持仓 gross/net/浮盈字段覆盖，以及公开来源正文缺失等回归和留出问法。离线行为清单只引用固定合成测试；通过仍不等于真实模型、真实搜索或生产业务验收通过。
