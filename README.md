...
- WH6 成交与持仓采集器 V2.1（Staging 开发版）：在全量只读采集、实时优先、本地断网/重启保留和服务端设备绑定基础上，按账户、环境和当前有效完整月结单保护已结算月份；日期晚于服务器当天的有效成交不再被限制。日结/月结按字段优先级协调并保留追加审计，上传按 100 条分批逐条确认，成交明细支持真正的 20/50/100 服务端分页。客户端版本源固定为 `0.3.1`，每次远程采集前自动上报版本；验证码自动决定测试版或正式版，界面不提供环境地址选择。策略不可用时只继续当前交易日本地排队，历史暂停。跨期组合的真实 WH6 原始编码尚未取得，解析状态保持 `unknown_format`，不凭猜测拆腿或生成夹具。Windows 构建直接输出单个便携 EXE；0.3.1 的 Windows/WH6/Staging 实机验收状态以最新发布记录和交接证据为准。
