...
def test_import_merges_manual_record_with_same_contract_key_and_preserves_management(tmp_path, monkeypatch):
    use_temp_db(tmp_path, monkeypatch)
    excel_sample = next(record for record in parse_order_finance_directory(LEDGER_DIR)["records"] if record["purchase_contract_no"])
    ...
    import_order_finance_directory(LEDGER_DIR, imported_by="pytest")
    merged = next(item for item in list_order_finance_records() if item["id"] == manual["id"])

    assert merged["source_file"].endswith(".xls")
