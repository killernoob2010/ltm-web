...
def replication_status(user=Depends(trading_collector_current_user)):
    require_permission(user, 'trading.collector', 'manage')
    from . import trading_collector_replication as replication
    return replication.get_status()
