...
未配置天勤认证或行情失败时，接口会使用同一 TXT 结算快照中的期权结算价和标的期货结算价，计算该结算日的浮动盈亏、IV 和 Greeks；页面展示估值价、标的价格、到期日、IV、四项希腊字母及估值日，不展示内部估值来源或状态。没有完整结算数据时不会用开仓价或零值伪装估值结果。实时行情、结算快照估值、浮动盈亏、IV 和 Greeks 只在接口响应中计算，不写入交易事实表。

期权持仓明细的德尔塔、伽马、西塔和维伽采用带买卖方向的每手口径，不乘持仓手数或合约乘数；西塔表示每日时间衰减（年化原值除以 360），维伽表示 IV 变化 1 个百分点的价格敏感度（原值除以 100）。组合汇总按每手值乘剩余手数聚合，同样不乘合约乘数。
