...
- WH6 期权成交采集器（Staging 开发版）：只读解析已绑定宏源账户的 `match.dat`，本地 SQLite 断网排队，服务端设备绑定与多设备去重；Windows 自包含安装包和真实 WH6 验收按 `docs/superpowers/plans/2026-09-02-wh6-windows-acceptance-runbook.md` 执行，当前 Mac 工作区不宣称已生成 `Setup.exe`。
