...
## 2026-09-15 正式部署后的真实验收失败（当前入口）

- 当前正式代码 `efb63f9`，由隔离分支 `codex/agent-batch-c-six-fixes-20260915-447c` 选择性推送 main；本地工作树 `/Users/wangjingze/.codex/worktrees/447c/轻量化交易管理系统WEB`。勿使用有无关脏文件的主目录 staging 作为发布来源。
- 用户授权正式部署测试及本轮新增模型费最多 ¥1。正式库九张 Agent 表已存在/RLS 开启，本次无迁移、业务事实写入、充值、升配或交易。
- SDK 字段投影修复已发布；本地 Agent 527 / 前端 28 通过。详见版本更新记录，不能替代业务验收。
- 正式管理员王景泽新问题 #137 确认 pydantic，4 次模型/4 次工具/0 搜索；真实 Call/Put 持仓已取数，但 invalid_reference 与需求覆盖失败导致空正文、failed。runAdminAcceptance 最终断言失败。#136 仍由 legacy 领取后 interrupted，不计新运行时通过。
- 计划误加 2024 年范围、浮盈指标与额外比较；工具范围匹配失败、事实引用格式错误。下一步先定位并修复这三个问题，再重新启用管理员试用，以新对话复验。连续追问/联网/重复矩阵尚未执行。
- `AGENT_V2_PYDANTIC_DISABLED=true` 已确认保存，Render `dep-dakibfgu01pc73f8gmng` 已 Live、健康及管理员入口正常；代码保留 efb63f9，代码回退点9471c4e。已有 pydantic 对话有粘性绑定，不能只改默认 backend 代替 kill switch。
- 费用：旧余额页面不可用作本轮基线；19:05新页面当天全账户消费0.45元，含此前调用且可能延迟，未有本轮精确金额。无新增模型请求，先核费用再安排付费复验。
- 发布/验收细节以 `版本更新记录.md` 最新条目为准；交易事实严格只读。
