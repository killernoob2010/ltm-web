...
(function () {
  ...
  async function loadData() {
    ...
    const replication = await api("/api/trading-collector/admin/replication-status");
    if (replication.environment === "production") {
      const fillState = replication.streams.find(item => item.stream === "fills");
      const message = !replication.enabled ? "服务端同步未启用" : fillState?.last_error || (fillState?.checked_at ? `上次同步检查 ${new Date(fillState.checked_at).toLocaleString("zh-CN", { hour12: false })}` : "等待首次同步");
      setStatus(`${message}｜已接收 ${replication.received_observations} 条采集观察记录（含重复来源）`);
    } else {
